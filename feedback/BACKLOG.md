...
_Last triaged: 2026-06-27. Source of these items: Erik's own field-notes/observations this session (a shipped-app feedback source like any other)._
...
|----|-------|--------|:-:|----------|------|-------|--------|-------|
| `fb-2026-06-27-prefs-first-render` | Preferences shows the wrong page on first open (recovers after switching tabs) | field-note | 1 | broken | bug | `bpl` | **in-progress** | Repro: open Preferences fresh → General is highlighted but the wrong tab's content renders; switch to another tab and back to recover. Root cause: `PreferencesWindowController` hosts the SwiftUI `TabView` with `sizingOptions = []` but **no explicit content size**, so the first tab never gets a correct first layout (sibling `NotebookWindowController` sets an explicit size and is fine). Fix built on branch `fix/prefs-first-render` (force layout + `setContentSize(fittingSize)`, 500×460 fallback; keeps `sizingOptions=[]` so no crash regression). **Awaiting PM visual verify on the dev build → then merge + cut v1.2.2.** |
