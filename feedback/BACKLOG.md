...
|----|-------|--------|:-:|----------|------|-------|--------|-------|
| `fb-2026-06-29-web-signin-stuck-loading` | Web sign-in (Google/Apple) hangs forever on "Loading" | field-note | 1 | broken | bug | `bpl` (auth/** — escalate merge) | **in-progress** | Account tab stuck at `.signingIn` (both buttons greyed + endless spinner) after clicking Sign in with Google. Root cause: both `WebGoogleAuthorizationProvider` + `WebAppleAuthorizationProvider` return a phantom `ASPresentationAnchor()` (an unshown NSWindow) from `presentationAnchor(for:)`; `ASWebAuthenticationSession` never presents/completes → continuation never resumes → permanent `.signingIn`. Backend reachable (ruled out). Fix: return the real visible window. Sensitive path (auth/**) → escalate merge. Not headlessly testable. Raw+shot: `feedback/raw/fb-2026-06-29-web-signin-stuck-loading.{md,png}`. |
